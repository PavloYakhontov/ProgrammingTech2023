document.querySelector('#dark-mode-toggle').addEventListener('click', () => {
    ...
    document.querySelector('meta[name="theme-color"]').setAttribute('content', isDarkMode ? '#1a1a2e' : '#adff2f');
});
...
const initGameGrid = () => {
    ...
    for (let i = 0; i < Math.pow(CONSTANT.GRID_SIZE,2); i++) {
        ...
        if (row === 2 || row === 5) cells[index].style.marginBottom = '7.5px';
        if (col === 2 || col === 5) cells[index].style.marginRight = '7.5px';
...
const  init = () => {
    ...
    document.querySelector('meta[name="theme-color"]').setAttribute('content', darkmode ? '#1a1a2e' : '#adff2f');
